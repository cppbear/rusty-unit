...
use crate::{RuConfig, HIR_LOG_PATH, INSTRUMENTED_MIR_LOG_NAME, LOG_DIR, LOG_EXT, MIR_LOG_NAME};
use serde::Serialize;
...
impl MirWriter {
    pub fn write(mir_object: &MirObject) {
        ...
        let path = Path::new(&RuConfig::env_crate_root())
            .join("analysis")
            .join("mir")
            .join("original")
            .join(file_name);
        ...
        file_writer::Writer::write(
            path.as_path(),
            serde_json::to_string(mir_object).as_ref().unwrap(),
        );
        #[cfg(feature = "redis_writer")]
        redis_writer::Writer::write(
            path.as_path(),
            serde_json::to_string(mir_object).as_ref().unwrap(),
        );
    }
    ...
    pub fn write_instrumented(mir_object: &MirObject) {
        let file_name = format!(
            "{}_{}.{}",
            INSTRUMENTED_MIR_LOG_NAME, &mir_object.global_id, LOG_EXT
        );
        let path = Path::new(&RuConfig::env_crate_root())
            .join("analysis")
            .join("mir")
...
impl HirWriter {
    pub fn write(hir_object: &HirObject) {
        let path = Path::new(&RuConfig::env_crate_root())
            .join("analysis")
            .join("hir")
            .join(format!("{}.{}", HIR_LOG_PATH, LOG_EXT));

        #[cfg(feature = "file_writer")]
        file_writer::Writer::write(
            path.as_path(),
            serde_json::to_string(hir_object).as_ref().unwrap(),
        );
        #[cfg(feature = "redis_writer")]
        redis_writer::Writer::write(
            path.as_path(),
