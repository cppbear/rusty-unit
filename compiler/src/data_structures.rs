use crate::rustc_data_structures::graph::Successors;
use crate::DOT_DIR;
use log::{debug, error};
...
pub fn truncated_cfg<'a>(
    body: &'a Body<'a>,
) -> (Graph<usize, usize>, HashMap<BasicBlock, NodeIndex>) {
    ...
    let basic_blocks = body.basic_blocks.reverse_postorder();
    ...
    table
        .entry(*bb0)
        .or_insert_with(|| graph.add_node(bb0.as_usize()));
...
pub fn successors_of_terminator_kind<'tcx>(
    ...
    match *kind {
        ...
        // InlineAsm { targets, .. } if *targets.is_empty() => {
        //     vec![]
        // }
        Goto { target: ref t }
        | Call {
            target: None,
            unwind: UnwindAction::Cleanup(ref t),
            ..
        }
        | Call {
            target: Some(ref t),
        ...
        InlineAsm { targets: ref t, .. } => {
            let mut ret_vec: Vec<BasicBlock> = Vec::new();
            for tt in t {
                if is_reachable(body, *tt) {
                    ret_vec.push(*tt);
                }
            }
            ret_vec
        ...
        Call {
            target: Some(ref t),
            unwind: UnwindAction::Cleanup(ref u),
            ..
        }
        | Yield {
            resume: ref t,
...
pub fn post_dominators(body: &Body<'_>) -> HashMap<usize, HashSet<usize>> {
    ...
    while changed {
        ...
        for block in &basic_blocks[1..] {
            ...
            let predecessors = body.basic_blocks.successors(*block);
...
pub fn visualize_graph<A: Display, B: Display>(graph: &Graph<A, B>, global_id: &str) {
    ...
    if let Ok(_) = log_graph_to(graph, dot_path.as_path()) {
        ...
        error!(
            "Could not write graph DOT file {}",
...
pub fn cdg(cfg: &Graph<usize, usize>) -> Graph<usize, usize> {
    ...
    let cfg_edges = cfg
        ...
        .collect::<Vec<_>>();
    let cfg_edges_debug = cfg_edges
        .iter()
        .map(|e| cfg.edge_endpoints(*e).unwrap())
        .collect::<Vec<_>>();
